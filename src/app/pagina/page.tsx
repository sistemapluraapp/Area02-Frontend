...
import { IdIcon, CertificateIcon } from '@/components/icons'
...
function PaginaDetalhe() {
  ...
  async function convidar(e: React.FormEvent) {
    e.preventDefault()
    if (!cpfConvite.trim()) return
    setConvidando(true)
    setErro('')
    try {
      await api.convidarColaborador(id, cpfConvite.trim())
      setCpfConvite('')
      await carregar()
    } catch (e) {
    ...
    <>
      ...
      <GlassCard style={{ marginTop: '1.5rem' }}>
          ...
          {pagina.vinculos.map((v) => (
            <div key={v.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.5rem 0.75rem', background: 'var(--c-glass-bg-sm)', borderRadius: '0.65rem' }}>
              <span style={{ fontSize: '0.875rem' }}>
                {v.usuario_id.slice(0, 8)}… <span style={{ color: 'var(--c-text-3)' }}>({v.papel})</span>
              </span>
              {v.papel === 'colaborador' && (
        ...
        <form onSubmit={convidar} style={{ display: 'flex', gap: '0.5rem', alignItems: 'flex-end' }}>
          <div style={{ flex: 1 }}>
            <Input label="Convidar por CPF" placeholder="000.000.000-00" value={cpfConvite} onChange={(e) => setCpfConvite(e.target.value)} leadingIcon={<IdIcon />} />
          </div>
            ...
            {pagina.certificados.map((cert) => (
              <div key={cert.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '0.5rem 0.75rem', background: 'var(--c-glass-bg-sm)', borderRadius: '0.65rem', fontSize: '0.875rem' }}>
                ...
                <span
                  style={{
                    fontWeight: 700,
                    color: cert.status === 'aprovado' ? '#22c55e' : cert.status === 'reprovado' ? '#ef4444' : 'var(--c-text-2)',
                  }}
                >
                  {cert.status}
                </span>
