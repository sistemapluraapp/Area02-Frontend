...
// Específico da Área 02: colaboradores são convidados pelo CPF.
export default function AbaEquipe({ rascunho: p, aplicarSalvo }: PropsAba) {
  ...
  async function convidar(e: FormEvent) {
    ...
    setErro('')
    try {
      const vinculo = await api.convidarColaborador(p.id, cpf.trim())
    ...
    <Secao titulo="Equipe" descricao="Colaboradores podem ajudar a manter a página. Só administradores alteram as informações.">
        ...
        {vinculos.map((v) => (
          <div key={v.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.625rem 0.875rem', borderRadius: '0.75rem', border: '1px solid var(--c-divider)' }}>
            <span style={{ fontSize: '0.9375rem' }}>
              {v.usuarios?.nome ?? `${(v.usuario_id ?? '').slice(0, 8)}…`} <span style={{ color: 'var(--c-text-3)', fontSize: '0.8125rem' }}>({v.papel})</span>
            </span>
            {v.papel === 'colaborador' && (
      ...
      <form onSubmit={convidar} style={{ display: 'flex', gap: '0.5rem', alignItems: 'flex-end', flexWrap: 'wrap' }}>
        <div style={{ flex: '1 1 240px' }}>
          <Campo rotulo="Convidar por CPF">
            <Texto valor={cpf} onChange={setCpf} placeholder="000.000.000-00" inputMode="numeric" />
          </Campo>
        </div>
        <button type="submit" disabled={enviando || !cpf.trim()} style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '0.65rem 1.125rem', borderRadius: '0.75rem', border: 'none', background: 'linear-gradient(135deg,#1a7aff,#0062e6)', color: '#fff', fontWeight: 600, fontFamily: 'inherit', cursor: 'pointer', opacity: enviando || !cpf.trim() ? 0.6 : 1 }}>
